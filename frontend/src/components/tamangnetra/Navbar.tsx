...
const navItems: NavItem[] = [
  ...
  { label: 'Translate', href: '#tool-section' },
  { label: 'About', href: '#about-section' },
  { label: 'Map', href: '#language-map-section' },
  ...
  return (
    <motion.header
      ...
      style={{
        ...
        WebkitBackdropFilter: scrolled ? 'blur(24px) saturate(180%)' : 'blur(12px) saturate(100%)',
      }}
    >
      {/* Gradient line at bottom — always visible (1px subtle), stronger when scrolled */}
      <div className={`absolute bottom-0 left-0 right-0 h-[1px] bg-gradient-to-r from-transparent via-emerald-500/40 dark:via-emerald-400/30 to-transparent transition-opacity duration-300 ${scrolled ? 'opacity-100' : 'opacity-30'}`} />
      {/* Subtle border glow in dark mode */}
      {scrolled && (
        <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-emerald-500 via-teal-500 to-amber-500 opacity-60 dark:opacity-40 dark:shadow-[0_0_8px_rgba(16,185,129,0.15)]" />
      )}
      ...
      <nav className="mx-auto flex h-14 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          <motion.div
            className="flex size-8 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 shadow-md shadow-emerald-500/20"
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-base font-bold bg-gradient-to-r from-teal-500 to-amber-500 bg-clip-text text-transparent">
              Netra
            </span>
            <span className="hidden sm:inline text-xs text-muted-foreground">तामाङनेत्र</span>
          </div>
        </div>
          ...
          <>
            ...
            >
              <div className="flex flex-col gap-1 px-4 py-4">
                {navItems.map((item, i) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {activeSection === item.href && item.label !== 'Translate' && (
                ...
                <div className="my-2 h-px bg-gradient-to-r from-transparent via-emerald-500/30 to-transparent" />
                <div className="px-2 py-1 text-[10px] text-muted-foreground">
                  Press <kbd className="px-1 py-0.5 rounded bg-muted border border-border text-[9px]">Ctrl+K</kbd> for shortcuts
                </div>
              </div>
            </motion.div>
          </>
